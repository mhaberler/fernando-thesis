  ...
  while(1)
  {
    ...
    select
    {
      case can_rx_frame(c_rx_tx, frame):
      {
        ...
        else if ((frame.id >= (NG_HEARTBEAT & 0x780) && (frame.id <= ((NG_HEARTBEAT & 0x780) + 0x7f))))
        {
            // Recibimos heartbeat de otro nodo.
        }
        else
        {
          switch(frame.id)
          {
            ...
            case SYNC:
            case (SYNC & 0x780):               // Sync trae COBid 0x080 sin agregar Node-id
