...
void aplicacion ( streaming chanend c_application, chanend c_pwm, streaming chanend c_encoder[NUMBER_OF_MOTORS])
{
  ...
  for(i=0;i<N_PUERTOS_PWM;i++){
      od_write_data(Salidas_pwm_ix, 1+i, (ui, char[]), 4);
  }
...
//      if(Status & MSK_STATUS_ENCENDIDO){
      ...
      od_read_data(Target_velocity_ix, 0, (i, char[]), 4);
        if(i != voltajes[0]){
            voltajes[0]=i;
            cc=1;         // marcamos que hay modificación a los pwm
            // actualizamos salida dir
            if(i < 0){
                dirs_pwm |= uc;
                valores_pwm[0] = -i;
            }
        ...
        // actualizamos máscara del bit para siguiente DIR de salida PWM
        uc << 1;
        // vamos a seguir comprobando desde 2do objeto PWM, el primero queda manejado exclusivamente por
        // el objeto 0x60FF
        for(c=1;c<N_PUERTOS_PWM;c++){
            // leemos los objetos de salidas pwm (cada objeto ocupa 4 bytes)
            od_read_data(Salidas_pwm_ix, 1+c, (i, char[]), 4);
            if(i != voltajes[c]){
